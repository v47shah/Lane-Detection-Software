...
def make_coordinates(image, line_parameters):   # to provide coordinates to the previously specified average slope and av. intercepts
    slope, intercept = line_parameters
    y1 = image.shape[0]
    y2 = int(y1*3/5)
    x1 = int((y1 - intercept)/slope)  #Since y = mx+b is our lin equation
...
def display_lines(image, lines):   # to display lines on black image
...
def region_of_interest(image):
    ...
    # ( Computing the bitwise & of both images as we saw earlier in the theory
    # section, takes the & function of each homologous picture in both arrays,
    # ultimately masking the canny image to only show the region of interest traced
...
#*******************************************************************************
# Code to detect lanes for a single image provided (commented out):
#*******************************************************************************
...
#-----#  that have to be alligned in a single straight line for it to be considered in the Hoiugh mapping

# averaged_lines = average_slope_intercept(lane_image, lines)
# line_image = display_lines(lane_image, averaged_lines)
# combo_image = addWeighted(lane_image, 0.8, line_image, 1, 1)
